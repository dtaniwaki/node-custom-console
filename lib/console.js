...
CustomConsole = (function() {
  ...
  function CustomConsole(tag, options) {
    ...
    this.tag = tag;
    this.regexp = new RegExp("\\*(\\:(.+))?|\\b" + this.tag + "(\\:(.+))?\\b");
    if (options.formatter) {
      this.formatter = options.formatter;
    }
  }

  CustomConsole.prototype.enabled = function(severity) {
    var debug, match;
    debug = process.env.NODE_DEBUG || '';
    return !!((match = debug.match(this.regexp)) && CustomConsole.getDebugLevel(severity) >= CustomConsole.getDebugLevel(match[2] || match[4]));
  };
